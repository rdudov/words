...
class TestCachedTranslationModel:
    ...
    def test_cached_translation_model_has_required_fields(self):
        """Test that CachedTranslation model has all required fields."""
        assert hasattr(CachedTranslation, 'cache_id')
        assert hasattr(CachedTranslation, 'word')
        assert hasattr(CachedTranslation, 'source_language')
        assert hasattr(CachedTranslation, 'target_language')
        assert hasattr(CachedTranslation, 'translation_data')
        assert hasattr(CachedTranslation, 'cached_at')
        assert hasattr(CachedTranslation, 'expires_at')

    def test_cached_translation_model_has_correct_tablename(self):
        """Test that CachedTranslation model has the correct table name."""
        assert CachedTranslation.__tablename__ == "cached_translations"
...
class TestCachedValidationModel:
    ...
    def test_cached_validation_model_has_required_fields(self):
        """Test that CachedValidation model has all required fields."""
        assert hasattr(CachedValidation, 'validation_id')
        assert hasattr(CachedValidation, 'word_id')
        assert hasattr(CachedValidation, 'direction')
        assert hasattr(CachedValidation, 'expected_answer')
        assert hasattr(CachedValidation, 'user_answer')
        assert hasattr(CachedValidation, 'is_correct')
        assert hasattr(CachedValidation, 'llm_comment')
        assert hasattr(CachedValidation, 'cached_at')
        assert hasattr(CachedValidation, 'word')

    def test_cached_validation_model_has_correct_tablename(self):
        """Test that CachedValidation model has the correct table name."""
        assert CachedValidation.__tablename__ == "cached_validations"
...
class TestIndexesAndConstraints:
    ...
    @pytest.mark.asyncio
    async def test_cached_translations_table_has_word_index(self):
        """Test that cached_translations table has index on word."""
        engine = create_async_engine("sqlite+aiosqlite:///:memory:", echo=False)
        ...
        async with engine.begin() as conn:
            ...
            def check_index(connection):
                ...
                indexes = inspector.get_indexes('cached_translations')
                index_names = [idx['name'] for idx in indexes]
                return 'idx_cached_translations_word' in index_names

            has_index = await conn.run_sync(check_index)
            assert has_index, "Index 'idx_cached_translations_word' not found on cached_translations table"

        await engine.dispose()

    @pytest.mark.asyncio
    async def test_cached_translations_table_has_languages_index(self):
        """Test that cached_translations table has composite index on (source_language, target_language)."""
        engine = create_async_engine("sqlite+aiosqlite:///:memory:", echo=False)
        ...
        async with engine.begin() as conn:
            ...
            def check_index(connection):
                ...
                indexes = inspector.get_indexes('cached_translations')
                index_names = [idx['name'] for idx in indexes]
                return 'idx_cached_translations_languages' in index_names

            has_index = await conn.run_sync(check_index)
            assert has_index, "Index 'idx_cached_translations_languages' not found on cached_translations table"

        await engine.dispose()

    @pytest.mark.asyncio
    async def test_cached_translations_table_has_expires_index(self):
        """Test that cached_translations table has index on expires_at."""
        engine = create_async_engine("sqlite+aiosqlite:///:memory:", echo=False)
        ...
        async with engine.begin() as conn:
            ...
            def check_index(connection):
                ...
                indexes = inspector.get_indexes('cached_translations')
                index_names = [idx['name'] for idx in indexes]
                return 'idx_cached_translations_expires' in index_names

            has_index = await conn.run_sync(check_index)
            assert has_index, "Index 'idx_cached_translations_expires' not found on cached_translations table"

        await engine.dispose()

    @pytest.mark.asyncio
    async def test_cached_validations_table_has_word_index(self):
        """Test that cached_validations table has index on word_id."""
        engine = create_async_engine("sqlite+aiosqlite:///:memory:", echo=False)
        ...
        async with engine.begin() as conn:
            ...
            def check_index(connection):
                inspector = inspect(connection)
                indexes = inspector.get_indexes('cached_validations')
                index_names = [idx['name'] for idx in indexes]
                return 'idx_cached_validations_word' in index_names

            has_index = await conn.run_sync(check_index)
            assert has_index, "Index 'idx_cached_validations_word' not found on cached_validations table"

        await engine.dispose()

    @pytest.mark.asyncio
    async def test_cached_validations_table_has_direction_index(self):
        """Test that cached_validations table has composite index on (word_id, direction)."""
        engine = create_async_engine("sqlite+aiosqlite:///:memory:", echo=False)
        ...
        async with engine.begin() as conn:
            ...
            def check_index(connection):
                ...
                indexes = inspector.get_indexes('cached_validations')
                index_names = [idx['name'] for idx in indexes]
                return 'idx_cached_validations_direction' in index_names

            has_index = await conn.run_sync(check_index)
            assert has_index, "Index 'idx_cached_validations_direction' not found on cached_validations table"

        await engine.dispose()

    @pytest.mark.asyncio
    async def test_cached_translations_table_has_unique_constraint(self):
        """Test that cached_translations table has UNIQUE constraint on (word, source_language, target_language)."""
        engine = create_async_engine("sqlite+aiosqlite:///:memory:", echo=False)
        ...
        async with engine.begin() as conn:
            ...
            def check_constraint(connection):
                table = CachedTranslation.__table__
                for constraint in table.constraints:
                    if hasattr(constraint, 'columns'):
                        column_names = {col.name for col in constraint.columns}
                        if {'word', 'source_language', 'target_language'}.issubset(column_names):
                            return True
                return False

            has_constraint = await conn.run_sync(check_constraint)
            assert has_constraint, "UNIQUE constraint on (word, source_language, target_language) not found"

        await engine.dispose()

    @pytest.mark.asyncio
    async def test_cached_validations_table_has_unique_constraint(self):
        """Test that cached_validations table has UNIQUE constraint on (word_id, direction, expected_answer, user_answer)."""
        engine = create_async_engine("sqlite+aiosqlite:///:memory:", echo=False)
        ...
        async with engine.begin() as conn:
            ...
            def check_constraint(connection):
                table = CachedValidation.__table__
                for constraint in table.constraints:
                    if hasattr(constraint, 'columns'):
                        column_names = {col.name for col in constraint.columns}
                        if {'word_id', 'direction', 'expected_answer', 'user_answer'}.issubset(column_names):
                            return True
                return False

            has_constraint = await conn.run_sync(check_constraint)
            assert has_constraint, "UNIQUE constraint on (word_id, direction, expected_answer, user_answer) not found"
...
class TestTableCreation:
    ...
    @pytest.mark.asyncio
    async def test_create_all_tables_including_cache_without_errors(self):
        """Test that all tables including cache tables can be created without errors."""
        engine = create_async_engine("sqlite+aiosqlite:///:memory:", echo=False)
        ...
        assert "cached_translations" in table_names
        assert "cached_validations" in table_names

        await engine.dispose()

    @pytest.mark.asyncio
    async def test_cached_translation_and_validation_can_coexist(self):
        """Test that CachedTranslation and CachedValidation can coexist in the database."""
        engine = create_async_engine("sqlite+aiosqlite:///:memory:", echo=False)
        async_session = async_sessionmaker(engine, class_=AsyncSession, expire_on_commit=False)
        ...
        async with async_session() as session:
            ...
            # Create a cached validation
            cached_validation = CachedValidation(
                word_id=word.word_id,
                direction="forward",
                expected_answer="тест",
                user_answer="тест",
                is_correct=True
            )
            session.add(cached_validation)
            ...
            # Verify both exist
            trans_result = await session.execute(select(CachedTranslation))
            translations = trans_result.scalars().all()
            assert len(translations) == 1

            val_result = await session.execute(select(CachedValidation))
